<div class="flex items-center gap-4 mb-6 pb-4 border-b border-gray-100">
    ...
    <div>
        <h3 class="text-lg font-bold text-gray-800">Edit Ujian</h3>
        <p class="text-xs text-gray-400 mt-0.5">Ubah informasi ujian di bawah.</p>
    </div>
</div>
...
<form method="POST" :action="editUrl" class="space-y-4">
    ...
    <div>
        <label for="edit_ujian_title" class="block text-xs font-bold text-gray-700 uppercase tracking-wider mb-1.5">Judul Ujian</label>
        <input type="text" name="title" id="edit_ujian_title" required x-model="editData.title"
    ...
    <div>
        <label for="edit_ujian_instructions" class="block text-xs font-bold text-gray-700 uppercase tracking-wider mb-1.5">Instruksi Ujian</label>
        <div class="prose max-w-none">
            <textarea name="instructions" id="edit_ujian_instructions" required x-model="editData.instructions"
                      x-init="ClassicEditor.create($el).then(editor => {
                          editor.model.document.on('change:data', () => {
                              $el.value = editor.getData();
                              $el.dispatchEvent(new Event('input'));
                          });
                          $watch('editData.instructions', value => {
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
            <label for="edit_ujian_duration_minutes" class="block text-xs font-bold text-gray-700 uppercase tracking-wider mb-1.5">Durasi Ujian (Menit)</label>
            <input type="number" name="duration_minutes" id="edit_ujian_duration_minutes" min="1" required x-model="editData.duration_minutes"
                   class="w-full px-4 py-2.5 bg-white border border-gray-200 rounded-xl focus:outline-none focus:border-[#0c2b4d] focus:ring-1 focus:ring-[#0c2b4d] text-sm text-gray-800 transition-all shadow-sm">
            @if(old('_method') === 'PUT')
        ...
        <div>
            <label for="edit_ujian_due_date" class="block text-xs font-bold text-gray-700 uppercase tracking-wider mb-1.5">Batas Ujian (Due Date)</label>
            <input type="datetime-local" name="due_date" id="edit_ujian_due_date" required x-model="editData.due_date"
                   class="w-full px-4 py-2.5 bg-white border border-gray-200 rounded-xl focus:outline-none focus:border-[#0c2b4d] focus:ring-1 focus:ring-[#0c2b4d] text-sm text-gray-800 transition-all shadow-sm">
            @if(old('_method') === 'PUT')
