<x-app-layout>
    <x-slot name="header">
        {{ __('Mengerjakan Ujian: ' . $ujian->title) }}
    </x-slot>
    ...
    <div class="max-w-7xl mx-auto font-sans" x-data="{
        ...
        init() {
            ...
            this.$watch('currentQuestionIndex', value => {
                localStorage.setItem('active_q_ujian_{{ $ujian->id }}_att_{{ $attempt->id }}', value);
            });
        },
        saveAnswer(soalId, tipe, nilai) {
            fetch('{{ route('murid.learning-modules.ujians.save-answer', [$learningModule->id, $ujian->id]) }}', {
                method: 'POST',
                headers: {
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
            ...
            <div class="lg:col-span-2 bg-white rounded-2xl border border-gray-150 p-5 shadow-sm flex flex-col justify-center">
                <h2 class="text-base font-extrabold text-gray-800">{{ $ujian->title }}</h2>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
            ...
            <div class="lg:col-span-2 bg-white rounded-2xl border border-gray-100 p-6 shadow-sm min-h-[400px] flex flex-col justify-between">
                <form id="ujian-form" action="{{ route('murid.learning-modules.ujians.submit', [$learningModule->id, $ujian->id]) }}" method="POST">
                    @csrf
             ...
             <div x-show="showSubmitConfirmModal" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100" x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" class="bg-white rounded-2xl overflow-hidden shadow-2xl transform transition-all w-full max-w-md mx-auto z-10 border border-gray-100">
                 ...
                 <div class="p-6 text-center">
                     ...
                     <p class="text-xs text-gray-500 leading-relaxed px-4">
                         Apakah Anda yakin ingin menyelesaikan ujian ini? Jawaban Anda akan langsung dikirim dan tidak dapat diubah lagi.
                     </p>
                     ...
                     <div class="mt-6 flex justify-center gap-3">
                         ...
                         <button type="button" @click="document.getElementById('ujian-form').submit(); showSubmitConfirmModal = false;"
