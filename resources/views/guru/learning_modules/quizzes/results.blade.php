    ...
    <div class="max-w-7xl mx-auto font-sans" x-data="{
        ...
        <div class="bg-[#0c2b4d] text-white p-6 rounded-2xl shadow-lg border border-gray-150 mb-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
                ...
                <h2 class="text-xl font-bold mt-2.5">Rekap Nilai & Jawaban - {{ $quiz->title }}</h2>
                <p class="text-xs text-blue-100/70 mt-1 max-w-xl">Lihat hasil pengerjaan kuis murid, nilai otomatis, dan berikan penilaian manual untuk soal esai.</p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                                        ...
                                        @if($att->status !== 'in_progress')
                                            <button @click="
                                                ...
                                                gradeAttemptData = {
                                                    ...
                                                    actionUrl: '{{ route('guru.learning-modules.quizzes.grade-essay', [$learningModule->id, $quiz->id, $att->id]) }}',
                                                    answers: [
                                                        @foreach($att->answers as $ans)
                                                            {
                                                                ...
                                                                bobot: {{ $quiz->soals->firstWhere('id', $ans->bank_soal_id)->pivot->bobot ?? 1 }}
                                                            },
                                                        @endforeach
                                                    ]
                                                }
            ...
            @else
                <div class="text-center py-14 text-gray-405 italic">
                    Belum ada murid yang memulai kuis ini.
                </div>
            @endif
        </div>
