<div class="flex items-center gap-4 mb-6 pb-4 border-b border-gray-100">
    ...
    <div>
        <h3 class="text-lg font-bold text-gray-800">Tambah Kuis</h3>
        <p class="text-xs text-gray-400 mt-0.5">Lengkapi detail kuis baru di bawah.</p>
    </div>
</div>

<form method="POST" action="{{ route('guru.learning-modules.quizzes.store', $learningModule->id) }}" class="space-y-4">
    @csrf

    <div>
        <label for="create_quiz_title" class="block text-xs font-bold text-gray-700 uppercase tracking-wider mb-1.5">Judul Kuis</label>
        <input type="text" name="title" id="create_quiz_title" required value="{{ !old('_method') ? old('title') : '' }}"
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
            <label for="create_quiz_duration_minutes" class="block text-xs font-bold text-gray-700 uppercase tracking-wider mb-1.5">Durasi Pengerjaan (Menit)</label>
            <input type="number" name="duration_minutes" id="create_quiz_duration_minutes" min="1" required value="{{ !old('_method') ? old('duration_minutes') : '' }}"
                   class="w-full px-4 py-2.5 bg-white border border-gray-200 rounded-xl focus:outline-none focus:border-[#0c2b4d] focus:ring-1 focus:ring-[#0c2b4d] text-sm text-gray-800 transition-all shadow-sm">
            @if(!old('_method'))
        ...
        <div>
            <label for="create_quiz_due_date" class="block text-xs font-bold text-gray-700 uppercase tracking-wider mb-1.5">Batas Pengerjaan (Due Date)</label>
            <input type="datetime-local" name="due_date" id="create_quiz_due_date" required value="{{ !old('_method') ? old('due_date') : '' }}"
                   class="w-full px-4 py-2.5 bg-white border border-gray-200 rounded-xl focus:outline-none focus:border-[#0c2b4d] focus:ring-1 focus:ring-[#0c2b4d] text-sm text-gray-800 transition-all shadow-sm">
            @if(!old('_method'))
