...
<div x-data="{
    ...
    <form method="POST" action="{{ route('guru.learning-modules.store') }}" class="space-y-4">
        ...
        <!-- Kelas (Filtered dynamically by Mata Pelajaran & Tahun Ajaran) -->
        <div>
            ...
            <p x-show="(selectedSubjectId || selectedTaId) && availableClassrooms.length === 0" class="text-amber-600 text-xs mt-1 font-medium">
                Tidak ada kelas yang di-assign mata pelajaran ini dalam Kurikulum Kelas pada Tahun Ajaran terpilih.
            </p>
            @if(!old('_method'))
