    ...
    <div class="max-w-7xl mx-auto font-sans" x-data="{
        ...
        <div class="bg-[#0c2b4d] text-white p-4 sm:p-6 rounded-2xl shadow-lg border border-gray-150 mb-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
                ...
                <h2 class="text-xl font-bold mt-2.5">Kelola Pertanyaan - {{ $quiz->title }}</h2>
                <div class="wysiwyg-content wysiwyg-on-dark text-xs text-blue-100/90 mt-1 max-w-xl">{!! $quiz->instructions !!}</div>
            </div>
        ...
        <div class="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm">
            <h3 class="text-base font-bold text-gray-800 mb-1">Daftar Soal Kuis Aktif</h3>
            ...
            @if($attachedSoals->isNotEmpty())
                        ...
                        @foreach($attachedSoals as $index => $soal)
                            <div class="p-5 border border-gray-150 rounded-2xl flex flex-col md:flex-row justify-between items-start md:items-center gap-4 hover:border-blue-250 transition-colors">
                                ...
                                <div class="flex items-center gap-3 w-full md:w-auto justify-end flex-shrink-0 pt-3 md:pt-0 border-t md:border-t-0 border-gray-100">
                                    ...
                                    <!-- Detach Button -->
                                    <button type="button" 
                                            onclick="if(confirm('Hapus soal ini dari kuis?')) { document.getElementById('detach-form-{{ $soal->id }}').submit(); }"
                                            class="text-gray-400 hover:text-red-650 hover:bg-red-50 p-2 rounded-lg transition-all cursor-pointer" title="Lepaskan Soal">
                ...
                <!-- Hidden Detach Forms -->
                @foreach($attachedSoals as $soal)
                    <form id="detach-form-{{ $soal->id }}" action="{{ route('guru.learning-modules.quizzes.soals.detach', [$learningModule->id, $quiz->id, $soal->id]) }}" method="POST" class="hidden">
                        @csrf
                        @method('DELETE')
                    </form>
                @endforeach
            @else
                <div class="text-center py-10 text-gray-405 italic border border-dashed border-gray-200 rounded-2xl">
                    Belum ada soal ditambahkan ke kuis ini. Klik "Pilih Dari Bank Soal" untuk menambahkan.
                </div>
            @endif
        </div>
