<div class="flex items-center gap-4 mb-6 pb-4 border-b border-gray-100">
    ...
    <div>
        <h3 class="text-lg font-bold text-gray-800">Tambah Materi</h3>
        <p class="text-xs text-gray-400 mt-0.5">Lengkapi detail materi pembelajaran baru di bawah.</p>
    </div>
</div>

<form method="POST" action="{{ route('guru.learning-modules.materis.store', $learningModule->id) }}" enctype="multipart/form-data" class="space-y-4">
    @csrf

    <div>
        <label for="create_materi_title" class="block text-xs font-bold text-gray-700 uppercase tracking-wider mb-1.5">Judul Materi</label>
        <input type="text" name="title" id="create_materi_title" required value="{{ !old('_method') ? old('title') : '' }}"
    ...
    <div>
        <label for="create_materi_file" class="block text-xs font-bold text-gray-700 uppercase tracking-wider mb-1.5">Berkas Lampiran (Maks 10MB)</label>
        <input type="file" name="file" id="create_materi_file"
               class="w-full px-4 py-2.5 bg-white border border-gray-200 rounded-xl focus:outline-none focus:border-[#0c2b4d] focus:ring-1 focus:ring-[#0c2b4d] text-sm text-gray-800 transition-all shadow-sm">
        @if(!old('_method'))
