    ...
    <div class="max-w-7xl mx-auto font-sans" x-data="{
        ...
        <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-6">
            ...
            <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full lg:w-auto">
                <!-- Import Form -->
                <form action="{{ route('admin.manage.murids.import') }}" method="POST" enctype="multipart/form-data" class="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 bg-white p-3 sm:px-4 sm:py-2 border border-gray-100 rounded-xl shadow-sm w-full sm:w-auto">
                    @csrf
                            ...
                            <tbody class="bg-white divide-y divide-gray-100">
                                @foreach ($murids as $userObj)
                                    <tr class="hover:bg-gray-50/50 transition-all duration-150"
                                        x-show="(searchQuery === '' || 
                                                 {{ json_encode(strtolower($userObj->murid->namaLengkap ?? $userObj->name)) }}.includes(searchQuery.toLowerCase()) || 
                                                 ...
                                                 {{ json_encode(strtolower($userObj->murid->nisn ?? '')) }}.includes(searchQuery.toLowerCase()) || 
